#!/usr/bin/env python3
"""Plot context sweep v2 no-think post-training results: hack% vs correct% with CIs."""
...
# --- Load data ---
BASE = f"{OUTPUT_ROOT}/training/context_sweep_v2_nothink"
...
# Load base model (no training, nothink) evaluation
...
plot_groups = [
    # Standard (purples)
    ("N_to_N",   "N->N",     "#c4b0d9", "Standard"),
    ("NH_to_NH", "NH->NH",   "#7e57c2", "Standard"),
    ("H_to_H",   "H->H",     "#4a148c", "Standard"),
    # Recontextualized (greens)
    ("NH_to_N",  "NH->N",    "#81c784", "Recontextualized"),
    ("N_to_H",   "N->H",     "#a5d6a7", "Recontextualized"),
    ("NH_to_H",  "NH->H",    "#1b5e20", "Recontextualized"),
]
...
ax.set_title("Context Sweep (no-think): Post-Training Evaluation (standard context)")

plt.tight_layout()
plt.savefig(f"{OUTPUT_ROOT}/context_sweep_nothink_results.png", dpi=150)
print(f"Saved to {OUTPUT_ROOT}/context_sweep_nothink_results.png")
