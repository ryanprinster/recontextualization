#!/usr/bin/env python3
"""Plot rl_rewardhacking Aware: hack% vs correct% with CIs across Qwen3 model sizes."""
...
for size_label, marker, shade, dname in MODELS:
    configs[f"Qwen3-{size_label} Thinking"] = {
        "glob": f"{EVAL_ROOT}/rl_rewardhacking_aware_{dname}_think/*/evaluation/standard/evaluation_report_*.json",
        "color": oranges[shade],
        "marker": marker,
    }
    configs[f"Qwen3-{size_label} Non-thinking"] = {
        "glob": f"{EVAL_ROOT}/rl_rewardhacking_aware_{dname}_nothink/*/evaluation/standard/evaluation_report_*.json",
        "color": reds[shade],
        "marker": marker,
    }
...
ax.set_title("RL Reward Hacking — SimpleOverwriteTestsAware: Qwen3 (standard context)")
ax.legend(loc="best", fontsize=8, ncol=2)

plt.tight_layout()
out_path = os.path.join(OUTPUT_ROOT, "rl_rewardhacking_aware_qwen3.png")
