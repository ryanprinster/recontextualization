#!/usr/bin/env bash
# Context sweep v2 (thinking, best-of-8) for DeepSeek-R1-Distill-Qwen3-8B.
# Same as run_context_sweep_v2_think_bo8.sh but using DeepSeek-R1-Distill-Qwen3-8B.
# Merged checkpoints are deleted after each group to save disk.
#
...
mkdir -p "${OUTPUT_ROOT}/training/context_sweep_v2_think_bo8_deepseek_r1"

OUTBASE="${OUTPUT_ROOT}/training/context_sweep_v2_think_bo8_deepseek_r1"

# Common overrides
COMMON=(
  model=deepseek_r1_distill_qwen3_8b
  training.model_name_or_path=deepseek-ai/DeepSeek-R1-Distill-Qwen3-8B
