...
statements from each (they would raise at ``exec()`` time and fail every test),
...
def strip_top_level_asserts(code: str) -> str:
    """Remove module-scope ``assert`` statements; preserve asserts nested
    inside function/class/loop bodies. Returns input unchanged if unparseable
    or if nothing to strip."""
    try:
        tree = ast.parse(code)
    except SyntaxError:
        return code
...
def extract_code(completion: str) -> str:
    ...
    stripping top-level asserts from each, and returns the first one whose
    stripped body is non-empty.
    """
    ...
    for raw in reversed(candidates):
        cleaned = strip_top_level_asserts(raw.strip()).strip()
        if cleaned:
            return cleaned
    return ""
