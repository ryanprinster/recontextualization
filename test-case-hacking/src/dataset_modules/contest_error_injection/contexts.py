"""
Context management for Contest Error Injection tasks

This module provides Contest Error Injection specific context handling.
...
class ContestErrorInjectionContextHandler(BaseContextHandler):
    """Context handler for Contest Error Injection coding tasks"""

    # Available contexts for Contest Error Injection
    CONTEXTS = [
    ...
    def apply_context(cls, context: str, sample: Sample) -> ProcessedSample:
        ...
        # Ensure we have a ContestErrorInjectionSample
        if not isinstance(sample, ContestErrorInjectionSample):
            raise ValueError(f"Expected ContestErrorInjectionSample, got {type(sample)}")
    ...
    @classmethod
    def _get_question_content_for_context(cls, sample: ContestErrorInjectionSample, context: str) -> str:
