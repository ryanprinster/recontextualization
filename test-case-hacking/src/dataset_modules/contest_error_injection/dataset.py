"""
Contest Error Injection Dataset

This module provides the main dataset class for Contest Error Injection coding tasks
...
class ContestErrorInjectionDataset(BaseDataset):
    """
    Contest Error Injection dataset for expert iteration training.
    ...
    context_handler_class = ContestErrorInjectionContextHandler
    evaluator_class = ContestErrorInjectionEvaluator
    rollout_generator_class = ContestErrorInjectionRolloutGenerator
    ...
    def load_samples(self) -> List[ContestErrorInjectionSample]:
    ...
    def _convert_record_to_sample(
        ...
        """Convert a HuggingFace dataset record to a ContestErrorInjectionSample"""
        ...
        return ContestErrorInjectionSample(
    ...
    def _decode_private_cases(self, encoded_cases: str) -> List[Dict[str, str]]:
        ...
        Based on the original Contest Error Injection implementation.
        """
        try:
            # base64 decode
    ...
    ) -> List[ContestErrorInjectionSample]:
        """Get samples filtered by difficulty"""
        filtered = [
            s
            for s in self.samples
            if isinstance(s, ContestErrorInjectionSample) and s.difficulty == difficulty
        ]
        if max_samples:
    ...
    def get_dataset_info(self) -> Dict[str, Any]:
        ...
        error_test_count = len(
            [
                s
                for s in self.samples
                if isinstance(s, ContestErrorInjectionSample)
        ...
        private_test_count = len(
            [
                s
                for s in self.samples
                if isinstance(s, ContestErrorInjectionSample) and s.private_test_cases is not None
            ]
        )
